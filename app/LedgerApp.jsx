  ...
  const submitForm = (e) => {
    ...
    if (editingId) {
      setTransactions((prev) =>
        prev.map((t) =>
          t.id === editingId
            ? { ...t, date: form.date, categoryId: form.categoryId, amount: signed, note: form.note || catById[form.categoryId]?.name || "" }
            : t
        )
      );
    } else {
  ...
  return (
    <div style={styles.page}>
      <div style={styles.wrap}>
        ...
        <button style={styles.addBtn} onClick={() => (formOpen ? cancelForm() : setFormOpen(true))}>
          {formOpen ? "Cancel" : "+ Add transaction"}
        </button>

        {formOpen && (
          <form style={styles.form} onSubmit={submitForm}>
            {editingId && <div style={styles.editingLabel}>Editing transaction</div>}
            ...
            <button type="submit" style={styles.saveBtn}>{editingId ? "Save changes" : "Save transaction"}</button>
          </form>
        )}
            ...
            <div key={t.id} style={styles.txRow}>
              ...
              <div style={styles.txRight}>
                ...
                <div style={styles.txActions}>
                  <button type="button" style={styles.iconBtn} onClick={() => startEdit(t)} aria-label="Edit">Edit</button>
                  <button type="button" style={styles.iconBtn} onClick={() => deleteTransaction(t.id)} aria-label="Delete">Delete</button>
                </div>
              </div>
            </div>
          ))}
...
const styles = {
  ...
  txAmount: { fontFamily: "'IBM Plex Mono', monospace", fontSize: 14, whiteSpace: "nowrap" },
  txActions: { display: "flex", gap: 6 },
  iconBtn: {
    background: "none",
    border: "1px solid var(--line)",
    borderRadius: 4,
